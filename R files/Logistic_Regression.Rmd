...
Lojistik regresyon, ikili(binary) 0 veya 1 olarak kodlanmış verileri içerir ve modelden elde edeceğimiz tahminler olasılıksal olarak elde edilir.Bu yüzden veri setimizde ufakk bir ayarlama yaparak yanıt değişkenimizi "0","1" olarak kodladık.
...
Modelimizi train seti üzerine kurup modelinin test verisi üzerindeki performansını inceleyelim.

```{r warning=FALSE}
set.seed(1)

fit_log <- glm(class ~ . , family = binomial, data = ups_training)

summary(fit_log)
```

Model çıktısı incelendiğinde psikolojik rahatsızlık tanısının tahmin edilmesinde, p value değerleri < 0.05 değişkenlerinin önemli olduğu sonucuna varılır.


## Tahmin

Test Set Tahmin Performansını inceleyelim ;

```{r echo=FALSE}
predictions_test <- predict(fit_log, test, type = "response")
pred_test        <- ifelse(predictions_test > 0.5, 1, 0)


cm_logistic_test <- caret::confusionMatrix(table(pred_test,test_y))
```
...
```{r}
draw_confusion_matrix(cm_logistic_test)
```
